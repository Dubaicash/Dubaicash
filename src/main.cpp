...
uint256 hashGenesisBlock("0x00000aa634939ebe3e2c36c34c3fac59486e75bfb1a5c2e5501bde0823d5267f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dubaicash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dubaicash Signed Message:\n";
...
{
    // Dubaicash: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    /// debug print
     printf("Difficulty Retarget - Dark Gravity Wave\n");
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                else if (pindexLast->nHeight+1 >= 10) { DiffMode = 1; }
        }
        else {
...
{
    // Dubaicash: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000aa634939ebe3e2c36c34c3fac59486e75bfb1a5c2e5501bde0823d5267f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "Dubaicash";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 400 * 1000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("573148962fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519850575;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 51820;

        if (fTestNet)
        {
            block.nTime    = 1519675160;
            block.nNonce   = 51820;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x4361af4c03f69a2d92ef6517af7a320291215dd2efe4d9fec8ba57593c58c6dd"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xd2, 0xb6, 0xdb }; // Dubaicash: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DubaicashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DubaicashMiner : ProcessBlock, block not accepted");
    }
